...
var margin = {top: 180, right: 80, bottom: 20, left: 100};
...
var brush_height = 400;
var brush_width = 15;

var year_label_xoff = 550;
var year_label_yoff = 200;

var year_label_xoff_f1 = 796;
var year_label_xoff_f2 = 609;

var year_label_yoff_f1 = 183;
var year_label_yoff_f2 = 148;
...
var canvas_gamma_x = 80;
var canvas_gamma = 80;
function canvas_gamma_transform(gamma) {
    var gamma_sign = gamma > 0 ? 1: -1;
    var gamma_absval = Math.abs(gamma);
    console.log('gamma transform: ' + gamma_sign.toString() + ' ' + gamma_absval.toString());
    return gamma_sign * (90 - gamma_absval);
}

var canvas_gamma_scale = d3.scale.linear()
    .range([-90, 90])
    .domain([0, 1000])
;
...
d3.json('lineups.json', function(data) {
    ...
    function make_brush() {
        ...
        var brush_cell_height = parseInt(brush_height/(10));
        ...
        _.forEach(_.range(MAX_YEAR, MIN_YEAR, -1), function(year, year_counter) {
            ...
            child_svg.append('rect')
                ...
                .attr('cursor', 'pointer')
                .attr('x', function() {
                    return parseInt(year / 10 - 191) * brush_width;
                })
                .attr('y', function() {
                    return brush_cell_height * (year % 10);
                })
    ...
    function make_howto() {
        ...
        svg.append('text')
            .attr('class', 'howto-text')
            .attr('x', 190)
            .attr('y', 320)
            .style('font-size', 12)
            .text('HOWTO: use landscape mode.')
        ;
        ...
        svg.append('text')
            ...
            .attr('y', 340)
            .style('font-size', 12)
            .text('tilt up /down to move year.')
        ;
        ...
        svg.append('text')
            ...
            .attr('y', 360)
            .style('font-size', 12)
            .text('rapid up down to switch decade')
        ;
    ...
    make_row(data[1921], 1, false);
    make_row(data_delta[1921], 2, false, 'red', diff_yscale);
...
//        console.log(canvas_gamma);
  ...
  //          console.log(canvas_gamma);
        ...
        var avg_gamma = d3.mean(gamma_cache);
        if (gamma_cache.length == 20) {
            gamma_cache.shift();
        }
        gamma_cache.push(argval);

        if (did_switch == 0) {
            if (argval - avg_gamma > 30) {
                current_decade -= 10
                did_switch = 50;
            } else if (argval - avg_gamma < -30) {
                current_decade += 10;
                did_switch = 50;
            }
        } else {
        ...
        var dgamma = parseFloat(argval) - parseFloat(last_gamma);
        d3.select('.alpha-label').text('ALPHA: ' + parseInt(alpha));
        d3.select('.beta-label').text('BETA: ' + parseInt(beta));
        d3.select('.gamma-label').text('GAMMA: ' + parseInt(argval) + ' ' + highlighted_year);

        d3.select('.dgamma-label').text('DGAMMA: AVG-100 ' + parseInt(avg_gamma)+ ' ' + did_switch);
        ...
        var this_year = parseInt(beta_scale(argval)) + current_decade;
    ...
    canvas.addEventListener('mousemove', function(evt) {
        ...
        canvas_gamma_x = canvas_gamma_scale(mousePos.x);
        canvas_gamma = canvas_gamma_transform(canvas_gamma_x);

        d3.select('.simgamma-label').text('sim_gamma ' + parseInt(canvas_gamma) + ' => ' + parseInt(canvas_gamma_x));

        console.log(canvas_gamma_x, canvas_gamma);
        console.log(message);
        var ev = {alpha: 0, beta: 0, gamma: canvas_gamma};
