...
d3.csv('combined2017.csv', function(data) {
    ...
    var players = _.uniq(data, function(d) {
        return d.playerid;
    });
...
//    console.log('ndata', ndata);
    ...
    var make_path = function(playerid) {
         ...
         var t0 = _.map(ndata[playerid], function(d, i) {
             //return {x: d, y: i+1};
             return {y: 0, x: i+1};
         });

        var t = _.map(ndata[playerid], function(d, i) {
            //return {x: d, y: i+1};
            return {
                y: d-median_multiplier*median_values[i].median,
                x: i+1};
        ...
        var tot = 0.0;
        var tcum = _.map(ndata[playerid], function(d, i) {
            //return {x: d, y: i+1};
            tot += d-median_multiplier*median_values[i].median;
            return {
                y: tot,
                x: i+1};
        });
        ...
        lines_cum[playerid] = svg.append("path")
            .attr("d", line(t0))
            .attr("stroke", function () {
                return stroke_color[iscurrent[playerid]]; //bat_hue(k);
            })
            .attr("stroke-width", unhighlighted_stroke_width)
            .style('opacity', 1)
            .attr("fill", "none")
